---
title: "Claudian + baoyu-skills + ModelScope：打造免费AI绘图工作流"
source: "https://mp.weixin.qq.com/s/FvF4OOhm1ryWbPHj3jf9JQ"
author:
  - "[[大脑不是我的]]"
published:
created: 2026-04-17
description: "Claudian + baoyu-skills + ModelScope：打造免费AI绘图工作流在 AI 绘"
tags:
  - "clippings"
---
大脑不是我的 *2026年3月1日 00:15*

---

## Claudian + baoyu-skills + ModelScope：打造免费AI绘图工作流

在 AI 绘图领域，我们经常面临两个问题：高昂的 API 费用和复杂的配置流程。本文介绍一个完全免费的解决方案——将 **Claudian** 、 **baoyu-skills** 和 **ModelScope** 三者结合，使用阿里云的 Qwen-Image-2512 模型进行高质量的 AI 绘图。

## 🎯 为什么选择这个组合

- **Claudian** ：运行在 Obsidian 中的 AI 助手，可以直接操作 vault 中的文件
- **baoyu-skills** ：强大的技能库，提供图像生成、文章配图等功能
- **ModelScope** ：阿里云的 AI 模型平台，提供免费的 API 调用额度
- **Qwen-Image-2512** ：251 亿参数的高质量文生图模型，细节丰富

## 📦 Claudian 简介

Claudian 是一个运行在 Obsidian vault 中的 AI 助手，特点包括：

- ✅ **直接文件访问** ：可以读写 vault 中的 Markdown、图片等文件
- ✅ **技能系统** ：通过 baoyu-skills 扩展功能
- ✅ **沙箱限制** ：被限制在 vault 目录中执行，保证安全
- ✅ **命令执行** ：可以在 vault 中运行 bash 命令

**关键限制** ：Claudian 只能访问 vault 内的文件，无法访问系统级的 `~/.baoyu-skills/` 目录。

## 🔧 baoyu-skills 简介

baoyu-skills 是一套可安装的 AI 技能包，常用的技能包括：
...
### baoyu-skills 的两种安装方式

**用户级安装**2. ：
`~/.baoyu-skills/`- 所有项目共享
	- 需要手动同步到项目
**项目级安装**6. ：
`项目/.baoyu-skills/`- 仅当前项目可用
	- 优先级高于用户级

**配置优先级** ：项目级配置 > 用户级配置 > 默认值

## 🌟 ModelScope 配置步骤

### 1\. 获取 API Key

访问 ModelScope 官网，注册账号并获取 API Key。

### 2\. 在 vault 中配置

在 vault 根目录创建 `.baoyu-skills/.env` 文件：
...
```
# 基础使用
cd C:/Users/nll/.claude/skills/baoyu-image-gen
npx -y bun scripts/main.ts \
  --prompt "一只可爱的橘猫坐在窗台上" \
  --image cat.png \
  --ar 1:1

# 封面生成（ Claudian 可以直接调用）
npx -y bun scripts/main.ts \
...
## ⚠️ 遇到的问题：Claudian 的沙箱限制

### 问题描述

**场景** ：我想让 Claudian 使用全局安装的 baoyu-skills，因为如果 skills 修改的话，项目级和全局 skills 会不一致。

**实际困难** ：Claudian 被限制在 vault 中执行，无法访问 vault 之外的文件，包括：

- ❌ `~/.baoyu-skills/` （用户级安装）
- ❌ `C:/Users/nll/.claude/skills/` （Claude 的技能目录）
- ✅ `vault/.baoyu-skills/` （项目级安装）
...
### 方案 1：在 vault 中使用项目级 baoyu-skills（推荐）
...
```
# 配置 API Key
echo 'MODELSCOPE_API_KEY=ms-your-key' > .baoyu-skills/.env
...
1. **创建共享技能仓库** ：
```
# 在 GitHub 上创建一个私有仓库 baoyu-skills-shared
git init
git add .
git commit -m "Initial commit"
git remote add origin https://github.com/your-username/baoyu-skills-shared.git
git push -u origin main
```
1. **在各个 vault 中添加为 submodule** ：
```
# 在 vault 根目录
...
```
# 在任何 vault 中
git submodule update --remote .baoyu-skills
```
...
### 方案 3：使用符号链接（需要 Claudian 支持）

**注意** ：目前 Claudian **不支持** 访问 vault 之外的符号链接，此方案仅供参考。

```
# 在 Windows 中
mklink /D .baoyu-skills C:\Users\nll\.baoyu-skills

# 在 macOS/Linux 中
ln -s ~/.baoyu-skills .baoyu-skills
```

**为什么不行** ：Claudian 会检测符号链接是否指向 vault 外部，如果是则拒绝访问。

### 方案 4：修改 Claudian 的配置（不推荐）

理论上可以通过修改 Claudian 的系统提示词来放宽路径限制，但这样做会：

- ❌ **降低安全性** ：AI 可能访问或修改系统文件
- ❌ **破坏设计** ：违背沙箱机制的设计初衷
- ❌ **难以维护** ：每次 Claudian 更新都可能失效
...
**方案 2（多项目协作）** ：使用 Git Submodule

```
vault-1/
├── .baoyu-skills/              # Git submodule
└── ...

vault-2/
├── .baoyu-skills/              # 同一个 submodule
└── ...
```

### 配置文件管理

创建一个配置模板 `.baoyu-skills-config-template.yml` ：

```
# baoyu-skills 配置模板
# 复制到各项目的 .baoyu-skills/ 目录使用
...
```
1. 请使用 baoyu-image-gen 生成一张封面，
   提示词保存在 dev/prompts/cover.md

2. 为这篇文章 [[笔记名称]] 生成小红书图片
...
**文章封面生成**2. ：
- 使用 Qwen-Image-2512 生成高质量封面
	- 文件大小约 1.5 MB，细节丰富
	- 生成时间约 3-4 分钟
**小红书信息图**5. ：
- 使用 baoyu-xhs-images 批量生成
	- 风格统一，适合社交媒体
	- 自动处理文字排版
**文章配图**8. ：
...
A：ModelScope API 是异步的，生成需要 2-4 分钟。检查以下内容：
...
# 重新生成，确保命令正确
npx -y bun scripts/main.ts --prompt "..." --image output.png
```
...
Claudian + baoyu-skills + ModelScope 是一个强大的免费 AI 绘图方案：

1. **Claudian** 提供 AI 助手和 vault 文件操作
2. **baoyu-skills** 提供丰富的图像生成技能
3. **ModelScope** 提供免费的 API 调用
4. **Qwen-Image-2512** 提供高质量输出

**关于沙箱限制** ：这是一个安全特性，我们应该 **适应** 它而不是 **绕过** 它。推荐的解决方案是在 vault 中使用项目级 baoyu-skills，或使用 Git Submodule 统一管理。
...
#Claudian #baoyu-skills #ModelScope #AI绘图
