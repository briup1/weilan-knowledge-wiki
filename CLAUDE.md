...
5. **综合（Synthesis）—— 此步骤不可省略**：主动审视新来源与现有知识体系之间的关系，产出领域全景或跨领域洞察。两个方向：
   - **领域内知识全景（优先）**：将同一领域的多个来源、entity、concept 汇总梳理，形成该领域的结构化知识地图。包括：模块依赖关系、能力分层、推荐学习路径、工具组合方案、技术演进脉络。让读者能从一张图中看到整个领域的全貌，而非散落在几十个独立页面中的碎片。
   - **跨领域分析对比**：当新来源与不同领域的现有知识形成有意义的关联时（如工具选型对比、架构演进分析、方法论冲突、同一概念在不同领域的差异化实现）。
