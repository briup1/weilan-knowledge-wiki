...
- 来源：https://mp.weixin.qq.com/s/qvzcyGgjBNjzqo3cIyY1kg
- 公众号：CodeAgent
- 发布时间：2026-06-11
- 抓取日期：2026-08-05
...
如下图：
![](pi-tool-call-lifecycle-assets/01.png)
...
![](pi-tool-call-lifecycle-assets/02.png)
...
-
B. 模型只是生成了一段文本("我想用 bash 跑 ls")，程序看到这段文本，替它跑了 `ls`，再把结果放回上下文给它读。
...
生成回复（可能含"我想调 bash ls"的意图）

识别

程序

从回复里 `filter` 出 `toolCall`
...
沿着代码看三步就能说明：
1.
`currentContext` 是 `runLoop` 的对象——它是作为参数传进`executeToolCalls` 的，主人是 `runLoop`。

1.
`newMessages`（归档用的数组）并没有传给`executeToolCalls`——它在参数列表之外，执行器无法直接访问。

1.
`executeToolCalls` 的职责是"执行"，不是"管状态存到哪几个数组里"。
...
```
读出来的策略是：
![](pi-tool-call-lifecycle-assets/03.png)
...
![](pi-tool-call-lifecycle-assets/04.png)
...
![](pi-tool-call-lifecycle-assets/05.png)
...
![](pi-tool-call-lifecycle-assets/06.png)
...
它有三个特征：
1.
可选。开头一句 `if (config.beforeToolCall)`——没挂这个 hook，整段跳过，默认放行。pi 默认不拦任何工具。

1.
被拦的结果和 banana 一样——`kind: "immediate"` + `createErrorToolResult` + `isError: true`，又是一条错误结果。

1.
它只是个拦截点，本身不含任何判断逻辑——它只会问"外部：拦不拦？"，认 `block: true/false`。判断逻辑在外部挂进来的函数里。
...
![](pi-tool-call-lifecycle-assets/07.png)
...
![](pi-tool-call-lifecycle-assets/08.png)

这一篇有两个要点：
1.
错误不直接 `throw` 终止，而是转成结果放回模型。这是 agent 可以自我修正并持续推进的基础。四条错误路径汇到同一个出口，模型仍可继续决策。

1.
机制和策略分离——loop 提供拦截点(`beforeToolCall`)，具体拦不拦的策略由上层填。这让同一个 loop 适配只读、交互、自动化等不同的权限场景。
