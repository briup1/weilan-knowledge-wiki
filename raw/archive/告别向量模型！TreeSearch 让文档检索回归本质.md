---
title: "告别向量模型！TreeSearch 让文档检索回归本质"
source: "https://mp.weixin.qq.com/s/k2HHfziaAoQUF_FVWfrRMg"
author:
  - "[[AI小新]]"
published:
created: 2026-04-18
description: "一个革命性的文档检索系统，零依赖、毫秒级延迟、完全离线，不需要向量化"
tags:
  - "clippings"
---
AI小新 *2026年4月16日 13:45*
...
TreeSearch 是一个 **结构感知的文档检索系统** ，由开源开发者 shibing624 创建。它提供了一种完全不同于传统 RAG 的文档检索方案。
...
CREATE VIRTUAL TABLE fts_nodes USING fts5(
    ...
    body,            -- 权重: 10.0 (正文最重要)
    summary,         -- 权重: 2.0 (摘要)
    code_blocks,     -- 权重: 1.0 (代码块)
    front_matter     -- 权重: 2.0 (元数据)
);
```
...
```
from treesearch import set_config

set_config(
    cjk_tokenizer='jieba',
    fts_title_weight=10.0,
    fts_body_weight=5.0
)
```
...
| 场景 | Elasticsearch | TreeSearch |
| --- | --- | --- |
| 单关键词查询 | 10-50ms | 5-20ms |
| 复杂布尔查询 | 50-200ms | 10-50ms |
| 1000万文档 | 100-500ms | 50-200ms |
...
| 格式 | 解析方式 |
| --- | --- |
| Markdown | 基于 H1-H6 标题层级 |
| Python | 基于 AST（类、函数、方法） |
| 多语言代码 | TreeSitter（支持 40+ 种语言） |
| JSON | 基于 JSON 结构 |
| CSV | 基于行列结构 |
| XML | 基于 XML 元素 |
| PDF | 可选（需要依赖） |
...
ts = TreeSearch("project_root/", db_path="code_index.db")

# 搜索认证相关的代码
results = ts.search("user authentication")
...
```
ts = TreeSearch("docs/", db_path="docs_index.db")
...
ts = TreeSearch("knowledge_base/", db_path="kb_index.db")

def retrieve_context(query: str, top_k: int = 5):
    """为 LLM 检索上下文"""
    results = ts.search(query, top_k_docs=3, max_nodes_per_doc=top_k)
    
    # 组织成 LLM 可用的格式
    context = "\n\n".join([
        f"[{r.doc_name}] {r.title}\n{r.text}"
        for r in results
    ])
...
```
向量化成本：1000万文档 × $0.02/1000 tokens = $200,000
向量DB 成本：$1,000-10,000/月
总成本：$200,000-$340,000（首年）+ $12,000-120,000/年（运维）
```

**TreeSearch 方案** ：

```
索引构建成本：单机 CPU 10-30 小时（无额外成本）
SQLite 数据库：50-100GB（存储成本 $100-200）
运维成本：增量更新自动化，成本接近零
总成本：$100-200（存储）+ 人力维护（低）
```
...
| 数据集规模 | 预期时间 | 瓶颈 |
| --- | --- | --- |
| 100 个 Markdown 文件 | < 1s | 解析器分发 |
| 1,000 个代码文件 | < 10s | AST 解析 |
| 5,000 个文档 | < 60s | 磁盘 I/O |
...
| 10 个文档 | < 5ms | FTS5 查询 |
| 100 个文档 | < 20ms | 节点评分 |
| 1,000 个文档 | < 100ms | 文档聚合 |
| 5,000 个文档 | < 200ms | 并发评分 |
...
```
# AND 查询
results = ts.search("民法典 AND 合同")

# OR 查询
results = ts.search("认证 OR 授权")

# NOT 查询
results = ts.search("authentication NOT oauth")

# 短语匹配
results = ts.search('"用户认证"')

# 通配符
results = ts.search("auth*")
```
