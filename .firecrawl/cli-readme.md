...
\| \`--agent \` \| \`-a\` \| AI agent ID \| 自动检测第一个可用 agent \|
\| \`--output \` \| \`-o\` \| 输出文件路径 \| 自动保存为 \`<输入文件名>.html\`，stdin 输入时输出到 stdout \|
\| \`--output-dir \` \| \`-d\` \| 自动保存目录 \| 当前目录 \|
\| \`--model \` \| — \| 使用的模型 \| agent 默认模型 \|
\| \`--format \` \| — \| 输入格式：markdown, text, csv, json \| markdown \|
...
\| 参数 \| 简写 \| 说明 \| 默认值 \|
\|------\|------\|------\|--------\|
\| \`--template \` \| \`-t\` \| 模板 ID \| 配置中的 default-template \|
...
html-anything config set-default-template  # 设置默认模板
html-anything config set-default-agent  # 设置默认 agent
html-anything config set-model  # 设置默认模型
